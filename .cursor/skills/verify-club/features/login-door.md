...
- `chips-open` keeps `/` on the chips landing with a `Sign in` control.
...
- Open `/` to confirm the chips landing is still there.
...
- Doctor reports a healthy instance at the printed URL (`/` is chips; `/example` redirects to `/demo`; `/demo` is the seed; `/club` redirects to `/login`; `/login` is Sign in + Email).
- Viewport width ≥ 1280.
...
- **Chips stays landing.** Navigate to `/`. The page shows `tech@nyu chips` and a `Sign in` control. There is no applicant list.
