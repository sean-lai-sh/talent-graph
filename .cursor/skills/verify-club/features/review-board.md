...
- `board-load` renders the seed board on `/` and `/example` with brand `Tech@NYU` and region `Applicants`.
...
- Open `/` in the browser.
- Open `/example` in the browser (same public board).
...
- **Load home.** Navigate to `/` on the doctor URL. Snapshot. The page shows `Tech@NYU`, region `Applicants`, searchbox `Search applicants`, groups `Needs review` and `Decided`, and a case heading (a seed name).
- **Load example door.** Navigate to `/example`. Snapshot. The same brand, Applicants region, and seed names are present. This is a second entry point, not a shortcut for `/`.
...
- **Proof.** Save accessibility snapshots and screenshots of (1) the loaded `/` board with Applicants visible, (2) Cleo's case with signal 7, (3) Ife's Insufficient Evidence. Put them in `evidence/<run-id>/review-board/`.
